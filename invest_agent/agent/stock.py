...
from invest_agent.agent.prompt.stock.functions_trade import STOCK_BUY, STOCK_SELL
from invest_agent.agent.prompt.stock.trade_time import KRX_TRADE_TIME, NXT_TRADE_TIME, SOR_TRADE_TIME, ORDER_CODE, NEED_PRICE, NO_NEED_PRICE
from invest_agent.agent.prompt.stock.prompt import INITIAL_GUIDE_FORMAT, STOCK_INFO_AUGMENTATION_PROMPT
...
class StockAgent:
    def __init__(self, model: str):
        ...
        from invest_agent.agent.prompt.stock.prompt import SYSTEM_PROMPT
    ...
    def generate(self, prompt: str, curr_datetime: str = ""):
        # DB 검색
        stock_info = self.db_agent.query_stock(prompt)["full_info"]
        
        print(stock_info)
        
        stock_info_str = STOCK_INFO_AUGMENTATION_PROMPT.format(
            curr_info=stock_info,
            last_info=self.last_db_info,
        )
        
        if len(stock_info) > 0:
            self.last_db_info = stock_info
        
        prompt = prompt + stock_info_str
        print(prompt)
...
if __name__ == "__main__":
    agent = StockAgent(model="gpt-4.1-nano")
    agent.reset()
